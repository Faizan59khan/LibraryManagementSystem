...
public class Forgot extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
		...
		PreparedStatement st = con.c.prepareStatement(sql);  // we use prepared statement when we use parameterized query

		st.setString(1, t1.getText());
		ResultSet rs = st.executeQuery();

		while (rs.next()) {
                    t2.setText(rs.getString("name")); // to jump col to col and to set value 
		...
		while (rs.next()) { //answer match to wo pass dedega
